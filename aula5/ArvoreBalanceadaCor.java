...
class AVLTree3 {                            // início da classe que implementa a árvore AVL
    ...
    static class Node {                     // classe interna que representa um nó da árvore
        String valor;                       // campo que guarda o valor String armazenado no nó
    ...
    public boolean buscar(Node node, String valor) { // busca recursiva de valor (começa geralmente pela raiz)
        ...
        if (valor.equals(node.valor)) return true; // se valor igual ao do nó, encontrou
        if (valor.compareTo(node.valor) < 0) return buscar(node.esquerda, valor);
...
// Teste
public class ArvoreBalanceadaCor {               // classe com método main para testar a AVLTree2
    public static void main(String[] args) {
        AVLTree3 arvore = new AVLTree3();     // cria uma nova árvore AVL vazia

        String[] valores = {"verde", "azul", "bege", "cinza", "laranja", "vermelho", "branco", "amarelo"}; // vetor de valores a serem inseridos
        ...
        System.out.println("\nPesquisa.....................");
        // Testando busca
        System.out.println("Buscar verde? " + arvore.buscar(arvore.raiz, "verde"));
        // imprime true se encontrou "verde" na árvore

        System.out.println("Buscar preto? " + arvore.buscar(arvore.raiz, "preto"));
        // imprime false porque "preto" não foi inserido
    }
}
